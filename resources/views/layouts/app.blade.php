...
<body class="bg-gray-100 h-screen antialiased leading-none font-sans">
    <div id="app">
        <header class="bg-teal-700 py-3">
            <div class="container mx-auto flex justify-between items-center px-6">
                <div>
                    ...
                    <a href="{{ url('/') }}" class="text-lg font-semibold text-orange-300 no-underline">All Aboard Europe</a>
                </div>
        ...
        <div class="md:hidden absolute top-16 left-0 right-0 bg-gray-100 border-b border-teal-700 z-10 hidden">
            <nav class="space-x-4 text-teal-700  py-10">
                <a class="no-underline hover:underline text-teal-700 text-xs py-10" href="/">Home</a>
                <a class="no-underline hover:underline text-teal-700 text-xs  py-10" href="/blog">Blog</a>
                <a class="no-underline hover:underline text-teal-700 text-xs  py-10" href="/aboutUs" >About Us</a>
                <a class="no-underline hover:underline text-teal-700 text-xs py-10" href="/map" >Map</a>
                <a class="no-underline hover:underline text-teal-700 text-xs  py-10" href="/contactUs" >Contact Us</a>
                <a class="no-underline hover:underline text-teal-700 text-xs  py-10" href="/gallery" >Gallery</a>
                 
                @guest
                    <a class="no-underline hover:underline text-teal-700 text-xs py-10" href="{{ route('login') }}">{{ __('Login') }}</a>
                    @if (Route::has('register'))
                        <a class="no-underline hover:underline text-teal-700 text-xs py-10" href="{{ route('register') }}">{{ __('Register') }}</a>
                    @endif
                @else
                    <span>{{ Auth::user()->name }}</span>

                    <a href="{{ route('logout') }}"
                       class="no-underline hover:underline text-teal-700 text-xs py-10"
                       onclick="event.preventDefault();
                            document.getElementById('logout-form').submit();">{{ __('Logout') }}</a>
                    <form id="logout-form" action="{{ route('logout') }}" method="POST" class="hidden">
                        {{ csrf_field() }}
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const dropdownMenu = document.querySelector('.md\\:hidden');
